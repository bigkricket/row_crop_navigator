#!/usr/bin/python3
"""
Gets the position of the crop and it commands to steer the wheels
Subscribes to 
    /crop/point_crop
...
class ChaseBall():
    def __init__(self):
        ...
        self.sub_center = rospy.Subscriber("/crop/point_crop", Point, self.update_ball)
    ...
    def update_ball(self, message):
        self.crop_x = message.x
        self.crop_y = message.y
        self._time_detected = time.time()
        # rospy.loginfo("Ball detected: %.1f  %.1f "%(self.crop_x, self.crop_y))
    ...
    def get_control_action(self):
        ...
        if self.is_detected:
            ...
            steer_action   =-K_LAT_DIST_TO_STEER*self.crop_x
